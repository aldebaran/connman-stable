...
int connman_inet_add_rule(const char* from, int index)
{
	...
	if ((err = add_rtattr(header, sizeof(request), FRA_SRC,
						...
						unsigned char prefix_len)
{
	...
	if (gateway != NULL) {
		...
		if ((err = add_rtattr(header, sizeof(request), RTA_GATEWAY,
	...
	if (host != NULL) {
		...
		if ((err = add_rtattr(header, sizeof(request), RTA_DST,
				      &ipv4_dst, sizeof(ipv4_dst))) < 0)
			goto out;

	}

	if ((err = add_rtattr(header, sizeof(request), RTA_OIF,
			      &index, 4)) < 0)
		goto out;
